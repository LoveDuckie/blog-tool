...
def get_log_filename() -> str:
    ...
    return f'blog_tool-log-{get_formatted_timestamp()}.log'
...
arg_parser_group_publish.add_argument(
    '--blog_tool', dest='blog_tools', type=str, action='append', required=False)
...
logger = logging.Logger(name='blog_tool-log')
...
def publish_blog():
    ...
    if parsed_args.blog_tools is None:
        raise ValueError("The list of blog_tools is invalid or null")

    if not isinstance(parsed_args.blog_tools, list):
        raise ValueError("The list of blog_tools is not a list.")

    if not any(parsed_args.blog_tools):
        raise ValueError("No blog_tools were defined.")
